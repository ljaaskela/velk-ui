...
class ScenePlugin final : public ::velk::ext::Plugin<ScenePlugin, ::velk::ui::IScenePlugin>
{
public:
    ...
    // IScenePlugin
    void for_each_scene(void (*cb)(::velk::ui::IScene*, void*), void* user) override
    {
        if (!cb) return;
        for (auto* s : scenes_) cb(s, user);
    }

    void register_scene(::velk::ui::IScene* scene) override
    {
        if (scene) scenes_.push_back(scene);
    }

    void unregister_scene(::velk::ui::IScene* scene) override
    {
...
private:
    ::velk::vector<::velk::ui::IScene*> scenes_;
};
